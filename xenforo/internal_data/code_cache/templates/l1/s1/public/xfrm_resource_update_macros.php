<?php
// FROM HASH: f0e496bd96bfab741e186d4231964e78
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
						...
						';
	if ($__templater->method($__vars['resource'], 'canViewTeamMembers', array()) AND ($__vars['update']['team_user_id'] AND ($__vars['update']['team_user_id'] != $__vars['resource']['user_id']))) {
		$__finalCompiled .= '
							<div class="message-lastEdit">
								' . 'Posted by' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('username_link', array($__vars['update']['TeamUser'], false, array(
			'defaultname' => $__vars['update']['team_username'],
		))) . '
							</div>
						';
	}
	...
	if ($__vars['update']['edit_count'] AND $__templater->method($__vars['update'], 'canViewHistory', array())) {
		$__compilerTemp4 .= '
												<a href="' . $__templater->func('link', array('resources/update/history', $__vars['update'], ), true) . '"
													class="actionBar-action actionBar-action--history actionBar-action--menuItem"
													data-xf-click="toggle"
													data-target="#js-resourceUpdate-' . $__templater->escape($__vars['update']['resource_update_id']) . ' .js-historyTarget"
