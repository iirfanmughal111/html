<?php
// FROM HASH: 13320321e0bbede94950f29fba57520e
return array(
...
<div class="block block--messages"
		...
		<div class="block-tabHeader tabs">
			<div class="hScroller" data-xf-init="h-scroller">
				<span class="hScroller-scroll">
					' . '
					<a href="' . $__templater->func('link', array('showcase/authors/reviews', $__vars['user'], $__vars['reviewTabs']['latest']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['reviewTabs']['latest']['selected'] ? 'is-active' : '') . '">' . 'Latest' . '</a>
					<a href="' . $__templater->func('link', array('showcase/authors/reviews', $__vars['user'], $__vars['reviewTabs']['helpful']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['reviewTabs']['helpful']['selected'] ? 'is-active' : '') . '">' . 'Most helpful' . '</a>
					<a href="' . $__templater->func('link', array('showcase/authors/reviews', $__vars['user'], $__vars['reviewTabs']['rating']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['reviewTabs']['rating']['selected'] ? 'is-active' : '') . '">' . 'Rating' . '</a>
					' . '
				</span>
			</div>
			...
			<div class="tabs-extra tabs-extra--minor">
				...
				<div class="menu menu--wide" data-menu="menu" aria-hidden="true"
					data-href="' . $__templater->func('link', array('showcase/authors/reviews-filters', $__vars['user'], $__vars['filters'], ), true) . '"
	...
	if ($__vars['filters']['rating']) {
		$__compilerTemp3 .= '
							<li><a href="' . $__templater->func('link', array('showcase/authors/reviews', $__vars['user'], $__templater->filter($__vars['filters'], array(array('replace', array('rating', null, )),), false), ), true) . '"
								...
								';
		if ($__vars['filters']['rating'] == 5) {
			$__compilerTemp3 .= '
									' . 'xa_sc_5_star_only' . '
								';
		} else if ($__vars['filters']['rating'] == 4) {
			$__compilerTemp3 .= '
									' . 'xa_sc_4_star_only' . '
								';
		} else if ($__vars['filters']['rating'] == 3) {
			$__compilerTemp3 .= '
									' . 'xa_sc_3_star_only' . '
								';
		} else if ($__vars['filters']['rating'] == 2) {
			$__compilerTemp3 .= '
									' . 'xa_sc_2_star_only' . '
								';
		} else if ($__vars['filters']['rating'] == 1) {
			$__compilerTemp3 .= '
									' . 'xa_sc_1_star_only' . '
								';
		} else if ($__vars['filters']['rating'] == 45) {
			$__compilerTemp3 .= '
									' . 'xa_sc_all_positive' . '
								';
		} else if ($__vars['filters']['rating'] == 123) {
			$__compilerTemp3 .= '
									' . 'xa_sc_all_critical' . '
								';
		}
	...
	if ($__vars['filters']['term']) {
		$__compilerTemp3 .= '
							<li><a href="' . $__templater->func('link', array('showcase/authors/reviews', $__vars['user'], $__templater->filter($__vars['filters'], array(array('replace', array('term', null, )),), false), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Remove this filter', array(array('for_attr', array()),), true) . '">
	...
	if ($__vars['filters']['order'] AND $__vars['sortOrders'][$__vars['filters']['order']]) {
		...
		if ((!$__vars['reviewTabs']['latest']['selected']) AND ((!$__vars['reviewTabs']['helpful']['selected']) AND (!$__vars['reviewTabs']['rating']['selected']))) {
			$__compilerTemp3 .= '
								<li><a href="' . $__templater->func('link', array('showcase/authors/reviews', $__vars['user'], $__templater->filter($__vars['filters'], array(array('replace', array(array('order' => null, 'direction' => null, ), )),), false), ), true) . '"
