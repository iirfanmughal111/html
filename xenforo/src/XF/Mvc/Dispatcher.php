...
class Dispatcher
{
	...
	{
		//$controllerClass instanceof \XF\Entity\Thread
		// if ($controllerClass != "XF:Thread"){
		// 	return parent::dispatchClass($controllerClass, $action,  $match, $controller ,  $previousReply );
		// }
	
		$params = $match->getParameterBag();

		// $tempAction = explode('-',$match->getResponseType());
		// if (isset($tempAction[1]) ){
		// 	$params = ['thread_id' => $tempAction[0]];	
		// }
	//	$action = 'Index';
		if (!$params)
		{
