...
use function array_key_exists, floatval, gettype, in_array, intval, is_array, is_object, is_scalar, is_string, strval;
...
class ValueFormatter
{
	public function decodeValueFromSource($type, $value)
	{
		...
		switch ($type)
		{
			...
			case Entity::LIST_ARRAY:
				throw new \LogicException("Do not know how to decode LIST_ARRAY type; choose a more specific type");
	...
	public function encodeValueForSource($type, $value)
	{
		...
		switch ($type)
		{
			...
			case Entity::LIST_ARRAY:
				throw new \LogicException("Cannot use LIST_ARRAY in scenario that requires encoding");
	...
	public function castValueToType($value, $type, array $columnOptions = [])
	{
		...
		switch ($type)
		{
			...
			case Entity::LIST_ARRAY:
				if (!is_array($value))
				{
					throw new \InvalidArgumentException("Attempted to convert " . gettype($value) . " to array list");
				}
				return $value;
	...
	)
	{
		...
		if (isset($columnOptions['allowedValues']) && !in_array($value, $columnOptions['allowedValues'], true))
		{
