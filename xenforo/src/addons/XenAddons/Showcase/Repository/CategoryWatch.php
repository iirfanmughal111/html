...
{
	public function setWatchState(\XenAddons\Showcase\Entity\Category $category, \XF\Entity\User $user, $action, array $config = [])
	{
		...
		$watch = $this->em->find('XenAddons\Showcase:CategoryWatch', [
		...
		switch ($action)
		{
			...
			case 'watch':
				if (!$watch)
				{
					$watch = $this->em->create('XenAddons\Showcase:CategoryWatch');
	...
	public function setWatchStateForAll(\XF\Entity\User $user, $action, array $updates = [])
	{
		...
		switch ($action)
		{
			case 'update':
				...
				return $db->update('xf_xa_sc_category_watch', $updates, 'user_id = ?', $user->user_id);

			case 'delete':
				return $db->delete('xf_xa_sc_category_watch', 'user_id = ?', $user->user_id);
