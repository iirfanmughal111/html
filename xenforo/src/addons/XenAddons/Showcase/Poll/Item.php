...
class Item extends AbstractHandler
{
	...
	public function canEdit(Entity $content, Poll $poll, &$error = null)
	{
		...
		if ($visitor->hasShowcaseItemCategoryPermission($content->category_id, 'editAny'))
		{
			return true;
		}
		...
		if ($content->isContributor() && $visitor->hasShowcaseItemCategoryPermission($categoryId, 'editOwn'))
		{
	...
	public function canAlwaysEditDetails(Entity $content, Poll $poll, &$error = null)
	{
		...
		$visitor = \XF::visitor();
		return ($visitor->user_id && $visitor->hasShowcaseItemCategoryPermission($content->category_id, 'editAny'));
	}
	...
	public function canVote(Entity $content, Poll $poll, &$error = null)
	{
		...
		return $visitor->hasShowcaseItemCategoryPermission($content->category_id, 'votePoll');
	}
	...
	public function getPollLink($action, Entity $content, array $extraParams = [])
	{
		if ($action == 'content')
		{
			return \XF::app()->router('public')->buildLink('showcase', $content, $extraParams);
		}
		else
		{
			return \XF::app()->router('public')->buildLink('showcase/poll/' . $action, $content, $extraParams);
		}
	}
