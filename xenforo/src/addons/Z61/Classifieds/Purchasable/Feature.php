...
use Z61\Classifieds\Entity\Package as PackageEntity;

class Feature extends AbstractPurchasable
{
	public function getTitle()
	{
		return \XF::phrase('z61_classifieds_listing');
	}
	...
	public function getPurchaseFromExtraData(array $extraData, \XF\Entity\PaymentProfile $paymentProfile, \XF\Entity\User $purchaser, &$error = null)
	{
		...
		if (!in_array($paymentProfile->payment_profile_id, $listing['purchasable']->payment_profile_ids))
		{
			$error = \XF::phrase('selected_payment_profile_is_not_valid_for_this_purchase');
			return false;
		}
	...
	public function getPurchaseObject(\XF\Entity\PaymentProfile $paymentProfile, $purchasable, \XF\Entity\User $purchaser)
	{
		...
		$purchase->returnUrl = $router->buildLink('canonical:classifieds/package-purchase');
	...
	{
		$purchaseRequest = $state->getPurchaseRequest();
		$listingId = $purchaseRequest->extra_data['user_upgrade_id'];
	...
	public function getPurchasablesByProfileId($profileId)
	{
		...
		$columnName = $finder->columnSqlName('payment_profile_ids'); // TODO: verify this works

		$router = \XF::app()->router('public');
		$listings = $finder->whereSql("FIND_IN_SET($quotedProfileId, $columnName)")->fetch();
		return $listings->pluck(function(\Z61\Classifieds\Entity\Listing $listing, $key) use ($router)
		{
			return ['z61_classifieds_' . $key, [
				'title' => $this->getTitle() . ': ' . $listing->title,
				'link' => $router->buildLink('classifieds/edit', $listing)
			]];
		}, false);
	}
}