        ...
        Crop.prototype = {
            init: function () {
                ...
                var defaultControls = $('<div/>', {'class': 'cropControls'})
                    ...
                    .append($('<button/>', {
                        'class': 'cropZoomIn',
                        'type': 'button'
                    }).on('click', $.proxy(this.zoomIn, this)))
                    .append($('<button/>', {
                        'class': 'cropZoomOut',
                        'type': 'button'
                    }).on('click', $.proxy(this.zoomOut, this)));
                ...
                if (typeof $.fn.hammer === 'function' || typeof Hammer !== 'undefined') {
                    ...
                    this.$image.on("dragstart", function () {
                        return false;
                    });
            ...
            updateOptions: function () {
                ...
                if (self.options.showControls === 'always' || self.options.showControls === 'auto' && is_touch_device())
                    ...
                    self.$frame.on('mouseenter.' + pluginName, function () {
                        self.$frame.addClass('hover');
                    });
                    self.$frame.on('mouseleave.' + pluginName, function () {
                        self.$frame.removeClass('hover');
                    });
                }
            ...
            drag: function (data, skipupdate) {
                ...
                var returnValue;
                if (this.options.onDragging && $.isFunction(this.options.onDragging)) {
                    returnValue = this.options.onDragging.call(null, {top: this.img_top, left: this.img_left});
                }
