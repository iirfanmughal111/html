...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canIgnoreUser', array($__vars['user'], ))) {
		$__compilerTemp1 .= '
				<a href="' . $__templater->func('link', array('members/ignore', $__vars['user'], ), true) . '"
					...
					data-xf-click="switch"
					data-sk-ignore="' . $__templater->filter('Ignore', array(array('for_attr', array()),), true) . '"
					data-sk-unignore="' . $__templater->filter('Unignore', array(array('for_attr', array()),), true) . '">
	...
	if ($__vars['xf']['visitor']['user_id'] AND ($__vars['xf']['visitor']['user_id'] != $__vars['user']['user_id'])) {
		$__compilerTemp2 .= '
			' . $__templater->button(' ' . (($__vars['vouch'] == 2) ? '<i class="fas fa-thumbs-down"></i> - ' : '<i class="fas fa-thumbs-up"></i> + ') . ' ' . 'fs_register_my_vouch', array(
			...
			' . $__templater->button('fs_register_review_me', array(
