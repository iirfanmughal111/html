...
class Rating extends AbstractHandler
{
	...
	{
		/** @var \XenAddons\Showcase\Entity\ItemRating $content */
		$item = $content->Item;

		$log->content_user_id = $content->user_id;
		$log->content_username = $content->User ? $content->User->username : '';
		$log->content_title = $item->title;
		$log->content_url = \XF::app()->router('public')->buildLink('nopath:showcase/review', $content);
	...
	{
		return \XF::phrase('xa_sc_item_review_in_x', [
