...
class Page extends AbstractHandler
{
	...
	{
		return \XF::phrase('xa_sc_item_page_x', ['title' => $title]);
	}
	...
	{
		return \XF::app()->router('public')->buildLink(($canonical ? 'canonical:' : '') . 'showcase/page', $entity);
	}
	...
	public function takeContentAction(Entity $entity, $action, array $options)
	{
		...
		else if ($action == 'delete')
		{
			...
			/** @var \XenAddons\Showcase\Service\ItemPage\Deleter $deleter */
			$deleter = \XF::app()->service('XenAddons\Showcase:ItemPage\Deleter', $entity);
	...
	{
		$visitor = \XF::visitor();
		return ['User', 'Item', 'Item.Category', 'Item.Category.Permissions|' . $visitor->permission_combination_id];
	}
}