...
class Preparer extends \XF\Service\AbstractService
{
	...
	public function setMessage($message, $format = true)
	{
		...
		$preparer->setConstraint('maxLength', $this->app->options()->xaScUpdateReplyMaxLength);
	...
	protected function getMessagePreparer($format = true)
	{
		...
		$preparer = $this->service('XF:Message\Preparer', 'sc_update_reply', $this->reply);
	...
	public function checkForSpam()
	{
		...
		switch ($decision)
		{
			...
			case 'denied':
				$checker->logSpamTrigger('sc_update_reply', null);
	...
	public function afterInsert()
	{
		...
		$checker = $this->app->spam()->contentChecker();
		$checker->logSpamTrigger('sc_update_reply', $this->reply->reply_id);
	}
	...
	protected function writeIpLog($ip)
	{
		...
		$ipEnt = $ipRepo->logIp($reply->user_id, $ip, 'sc_update_reply', $reply->reply_id);
		if ($ipEnt)
		{
