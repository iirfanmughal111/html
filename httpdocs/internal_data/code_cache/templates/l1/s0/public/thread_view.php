...
'thread_action_buttons' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
						';
	if ($__vars['xf']['options']['bump_thread_above_post']) {
		$__finalCompiled .= '	
							' . $__templater->includeTemplate('bump_thread_button', $__vars) . '
						';
	}
	$__finalCompiled .= '
...
';
	...
	$__compilerTemp2 .= '
											';
	if ($__vars['xf']['options']['bump_thread_menu']) {
		$__compilerTemp2 .= '	
												' . $__templater->includeTemplate('bump_thread_button_menu', $__vars) . '
											';
	}
	$__compilerTemp2 .= '
											';
	if ($__templater->method($__vars['thread'], 'canDisableBump', array())) {
		$__compilerTemp2 .= '
												<a href="' . $__templater->func('link', array('threads/bump-disable', $__vars['thread'], ), true) . '"
													class="menu-linkRow"
													data-xf-click="switch"
													data-menu-closer="true">

													';
		if ($__vars['thread']['bump_thread_disabled']) {
			$__compilerTemp2 .= '
														' . 'Allow thread bumping' . '
													';
		} else {
			$__compilerTemp2 .= '
														' . 'Disallow thread bumping' . '
													';
		}
		$__compilerTemp2 .= '
												</a>
