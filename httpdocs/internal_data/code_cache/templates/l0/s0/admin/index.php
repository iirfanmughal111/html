...
';
	if ($__templater->method($__vars['xf']['visitor'], 'hasAdminPermission', array('viewLogs', )) AND $__vars['serverErrorLogs']) {
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--error blockMessage--iconic">
		<a href="' . $__templater->func('link', array('logs/server-errors', ), true) . '"> ' . 'Server errors have been logged. You should review these.' . '</a>
	</div>
';
	}
	$__finalCompiled .= '
...
';
	...
	<div class="block">
		<div class="block-container">
			<h2 class="block-header"><a href="' . $__templater->func('link', array('stats', ), true) . '">' . 'Statistics' . '</a></h2>
...
' . '
		...
		$__finalCompiled .= '
					...
					<dl class="pairs pairs--columns">
						...
						<dd>' . ($__vars['envReport']['gmp'] ? 'Yes' : 'No') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . '<code>ZipArchive</code> support' . '</dt>
						<dd>' . ($__vars['envReport']['zip'] ? 'Yes' : 'No') . '</dd>
					</dl>
				</div>
			</div>
		</div>
