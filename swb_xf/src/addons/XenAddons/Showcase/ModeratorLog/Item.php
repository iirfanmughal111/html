...
class Item extends AbstractHandler
{
	...
	protected function getLogActionForChange(Entity $content, $field, $newValue, $oldValue)
	{
		switch ($field)
		{
			...
			case 'prefix_id':
				if ($oldValue)
				{
					$old = \XF::phrase('sc_item_prefix.' . $oldValue)->render();
				}
				else
				{
					$old = '-';
			...
			case 'category_id':
				$category = \XF::em()->find('XenAddons\Showcase:Category', $oldValue);
	...
	protected function setupLogEntityContent(ModeratorLog $log, Entity $content)
	{
		...
		$log->content_url = \XF::app()->router('public')->buildLink('nopath:showcase', $content);
