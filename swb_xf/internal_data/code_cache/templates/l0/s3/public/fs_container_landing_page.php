<?php
// FROM HASH: d851149b891da6a5f336caa760a6ce2f
return array(
...
';
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canSearch', array())) {
		...
		$__compilerTemp3 .= $__templater->formCheckBox(array(
			'standalone' => 'true',
		), array(array(
			'name' => 'c[title_only]',
			'label' => '
...
<div class="p-body">
	...
	$__compilerTemp8 .= '
						';
	$__compilerTemp9 = '';
	$__compilerTemp9 .= (isset($__templater->pageParams['pageAction']) ? $__templater->pageParams['pageAction'] : '');
	if (strlen(trim($__compilerTemp9)) > 0) {
		$__compilerTemp8 .= '
							<div class="p-title-pageAction">' . $__compilerTemp9 . '</div>
						';
	}
	$__compilerTemp8 .= '
					';
	...
	if (!$__templater->test($__vars['headerHtml'], 'empty', array())) {
		...
		';
	} else if (strlen(trim($__compilerTemp7)) > 0) {
		$__finalCompiled .= '
			<div class="p-body-header">
			' . $__compilerTemp7 . '
			</div>
