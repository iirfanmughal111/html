<?php
// FROM HASH: a9035c7d10b09f7b78a13e1164a9112c
return array(
...
';
	...
	' . $__templater->button('Add feed', array(
		'href' => $__templater->func('link', array('xa-sc/feeds/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
...
';
		...
		if ($__templater->isTraversable($__vars['feeds'])) {
			foreach ($__vars['feeds'] AS $__vars['feed']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->func('link', array('xa-sc/feeds/edit', $__vars['feed'], ), false),
				...
				array(
					'href' => $__templater->func('link', array('xa-sc/feeds/import', $__vars['feed'], ), false),
					'_type' => 'action',
					'html' => 'Import now',
				),
				array(
					'href' => $__templater->func('link', array('xa-sc/feeds/delete', $__vars['feed'], ), false),
					'_type' => 'delete',
					'html' => '',
				))) . '
					';
	...
	', array(
			'action' => $__templater->func('link', array('xa-sc/feeds/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
