...
class CustomersController extends Controller
{
	...
	public function customer_create_new(CreateCustomerRequest $request){
		...
		if($request->ajax()){
			...
			if(!empty(trim($request->tag_line))){
				$data['tag_line'] = trim($request->tag_line);
			}
			$dat = User::create($data);

			$user_id = $dat->id;
			if(!empty(trim($request->description)) || !empty(trim($request->rating)) || !empty(trim($request->facebook_link)) || !empty(trim($request->instagram_link)) || !empty(trim($request->twitter_link))){
				$profileData = array();
				if(!empty(trim($request->description)))
					$profileData['description'] = trim($request->description);

				if(!empty(trim($request->rating)))
					$profileData['rating'] = trim($request->rating);
	...
	public function update_customer(UpdateCustomerRequest $request,$customer_id){
		...
		$stored_data = User::with('userProfile')->where('id',$customer_id)->first();
