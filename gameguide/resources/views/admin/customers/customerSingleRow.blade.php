<tr data-customer-id="{{ $customer->id }}" class="user_row_{{$customer->id}}" >		
	...
	<td id="role_{{$customer->id}}">{{$customer->role->title}}</td>
	...
	<td id="action_{{$customer->id}}">
		...
		@if(check_role_access('customer_manage'))
			<a class="action" target = "_blank" href="{{url('admin/manage-customer')}}/{{$customer->id}}"  data-user_id="{{ $customer->id }}" title="Manage Customer"><i class="simple-icon-bubbles"></i> 
				{{-- Check if customer unread count greater then 0 --}}
				@php
					$unreadCounter = unreadMessageCount($customer->id);
				@endphp
