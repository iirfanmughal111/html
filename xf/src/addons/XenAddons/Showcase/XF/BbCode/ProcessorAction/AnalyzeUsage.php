...
namespace XenAddons\Showcase\XF\BbCode\ProcessorAction;
...
class AnalyzeUsage extends XFCP_AnalyzeUsage
{
	...
	public function addAnalysisHooks(AnalyzerHooks $hooks)
	{
		...
		$hooks->addTagHook('showcase', 'analyzeShowcaseTag');
	}

	public function getShowcaseEmbeds()
	{
		return $this->showcaseEmbeds;
	}

	public function analyzeShowcaseTag(array $tag, array $options, $finalOutput, Processor $processor)
	{
		...
		if ($type && $id)
		{
			$this->showcaseEmbeds[$type][$id] = $id;
		}
	}
}