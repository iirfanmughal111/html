<?php
// FROM HASH: d6260135ef2d618a4f5424b14d47db91
return array(
...
';
			...
			foreach ($__compilerTemp2 AS $__vars['id'] => $__vars['treeEntry']) {
				...
				if ($__vars['categoryWatch']) {
					$__compilerTemp1 .= '
						';
					$__compilerTemp3 = '';
					if ($__vars['categoryWatch']['notify_on'] == 'item') {
						$__compilerTemp3 .= '<li>' . 'New items' . '</li>';
					}
					$__compilerTemp4 = '';
					...
					$__vars['bonusInfo'] = $__templater->preEscaped('
						...
						');
					$__compilerTemp1 .= '
						' . $__templater->callMacro('xa_sc_category_list_macros', 'category', array(
	...
	', array(
			'action' => $__templater->func('link', array('watched/showcase-categories/update', ), false),
			'ajax' => 'true',
			'class' => 'block',
			'autocomplete' => 'off',
		)) . '
