<?php
// FROM HASH: b1c465bf2736085c4fae264af486c2a1
return array(
...
';
			...
			foreach ($__compilerTemp2 AS $__vars['id'] => $__vars['treeEntry']) {
				...
				if ($__vars['categoryWatch']) {
					$__compilerTemp1 .= '
						';
					$__compilerTemp3 = '';
					if ($__vars['categoryWatch']['notify_on'] == 'media') {
						$__compilerTemp3 .= '
									<li>' . 'New media' . '</li>
								';
					}
					...
					$__vars['bonusInfo'] = $__templater->preEscaped('
						...
						');
					$__compilerTemp1 .= '
						' . $__templater->callMacro('xfmg_category_list_macros', 'category', array(
	...
	', array(
			'action' => $__templater->func('link', array('watched/media-categories/update', ), false),
			'ajax' => 'true',
			'class' => 'block',
			'autocomplete' => 'off',
		)) . '
