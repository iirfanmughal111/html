<?php
// FROM HASH: 25559bacee804ba97af64e17ae1e5e0f
return array(
...
'extensions' => array('icon_cell' => function($__templater, array $__vars, $__extensions = null)
{
	...
	';
	if ($__templater->func('in_array', array($__vars['thread']['Forum']['node_id'], $__templater->filter($__vars['xf']['options']['fs_register_compainion_ids'], array(array('split', array()),), false), ), false)) {
		$__finalCompiled .= '	
			<style>
  ...
  position: relative;
	...
	';
		if ($__vars['thread']['FirstPost']['attach_count']) {
			$__finalCompiled .= '
	' . $__templater->callMacro('news_feed_attached_images', 'attached_images', array(
				'attachments' => $__vars['thread']['FirstPost']['Attachments'],
...
'main_cell' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__compilerTemp2 .= '
						';
	if ($__vars['xf']['options']['bump_thread_thread_list']) {
		$__compilerTemp2 .= '	
							' . $__templater->includeTemplate('bump_thread_thread_list_macros_button', $__vars) . '
						';
	}
	$__compilerTemp2 .= '
